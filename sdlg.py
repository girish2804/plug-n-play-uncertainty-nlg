# generation algorithm taken from SDLG
# original implementation: https://github.com/ml-jku/SDLG.git
...
def get_model_layer_count(model):
    """
    Get the total number of layers for different model architectures
    """
    model_name = model.config.model_type.lower() if hasattr(model.config, 'model_type') else ""
    ...
    if hasattr(model.config, 'num_hidden_layers'):
        ...
        # Default fallback based on model name/size heuristics
        if "7b" in model_name or "7B" in str(model.config):
            return 32
        elif "13b" in model_name or "13B" in str(model.config):
            return 40
        elif "70b" in model_name or "70B" in str(model.config):
            return 80
        else:
            return 32


# algorithm 2 (according to SDLG paper)
# ranks (original token , alternative token) pairs according to the three scores 
...
# algorithm 1 (according to SDLG paper)
# generates the diverse samples
def generate_semantically_diverse_output_sequences(results_dict, 
    ...
    with torch.no_grad():
        ...
        for i, s in enumerate(sorted_indices):
            ...
            layer_start = total_layers - args.num_top_layers
            candidate_premature_logits = {}
            candidate_premature_layers = [x for x in range(layer_start,total_layers)]
